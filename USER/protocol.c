...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_SWITCH_TOMATOCLOCK, DP_TYPE_BOOL},
  {DPID_TOMATOCLOCK_TIME, DP_TYPE_ENUM},
};
...
void uart_transmit_output(unsigned char value)
{
    ...
    Fn_UART3_SendChar((unsigned char)(value));
/*
    //Example:
...
void all_data_update(void)
{
    ...
    mcu_dp_bool_update(DPID_SWITCH_LED,当前On/Off); //BOOL型数据上报;
    ...
    mcu_dp_value_update(DPID_COUNTDOWN,当前Timer); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_SWITCH_TOMATOCLOCK,当前Tomato clock); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_TOMATOCLOCK_TIME,当前Reminder time); //枚举型数据上报;

    */
}

extern unsigned char vruc_OnOff;
extern unsigned int vruc_DIM;
extern unsigned int vruc_CCT;
...
static unsigned char dp_download_switch_led_handle(const unsigned char value[], unsigned short length)
{
    ...
    if(ret == SUCCESS){
      vruc_OnOff = (unsigned char)(switch_led);
      vruc_WIFI_OnOffUpdate = 1;
      Fn_UART1_Pustf("On/Off\n");
      return SUCCESS;
    }
    else{
      return ERROR;
...
static unsigned char dp_download_bright_value_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    ...
    if(ret == SUCCESS){
      vruc_WIFI_DIMUpdate = 1;
      vruc_DIM = (bright_value & 0x0FFF);
      if(vruc_DIM > 1000){
        vruc_DIM = 1000;
      }
      return SUCCESS;
...
static unsigned char dp_download_temp_value_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    ...
    if(ret == SUCCESS){
      vruc_WIFI_CCTUpdate = 1;
      vruc_CCT = (temp_value & 0x0FFF);
      if(vruc_CCT > 1000){
        vruc_CCT = 1000;
      }
      return SUCCESS;
...
/*****************************************************************************
Function name : dp_download_switch_tomatoclock_handle
Function description : on DPID_SWITCH_TOMATOCLOCK processing function
Input parameter : value:Source data
        : length:Data length
Return parameter : Successful return:SUCCESS/Failed to return:ERROR
Instructions for use : 可下发可上报type,need to report the result to App after data is dealt with
*****************************************************************************/
static unsigned char dp_download_switch_tomatoclock_handle(const unsigned char value[], unsigned short length)
{
    //Example: The current DP type isBOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char switch_tomatoclock;
    
    switch_tomatoclock = mcu_get_dp_download_bool(value,length);
    if(switch_tomatoclock == 0) {
        //开关关
    }else {
        //开关开
    }
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_SWITCH_TOMATOCLOCK,switch_tomatoclock);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
Function name : dp_download_tomatoclock_time_handle
Function description : on DPID_TOMATOCLOCK_TIME processing function
Input parameter : value:Source data
        : length:Data length
Return parameter : Successful return:SUCCESS/Failed to return:ERROR
Instructions for use : 可下发可上报type,need to report the result to App after data is dealt with
*****************************************************************************/
static unsigned char dp_download_tomatoclock_time_handle(const unsigned char value[], unsigned short length)
{
    //Example: The current DP type isENUM
    unsigned char ret;
    unsigned char tomatoclock_time;
    
    tomatoclock_time = mcu_get_dp_download_enum(value,length);
    switch(tomatoclock_time) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_TOMATOCLOCK_TIME, tomatoclock_time);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        break;
        case DPID_COUNTDOWN:
            //Timerprocessing function
            ret = dp_download_countdown_handle(value,length);
        break;
        case DPID_SWITCH_TOMATOCLOCK:
            //Tomato clockprocessing function
            ret = dp_download_switch_tomatoclock_handle(value,length);
        break;
        case DPID_TOMATOCLOCK_TIME:
            //Reminder timeprocessing function
            ret = dp_download_tomatoclock_time_handle(value,length);
        break;
